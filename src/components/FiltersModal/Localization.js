...
export const Localization = ({ addressesList, onFilterClick, address }) => {
    ...
    const onSearchedElementClick = (address) => {
        ...
        setResults([]);
        onFilterClick('address', address.city);
    };
    ...
    return (
        <FilterSection subTitle={'Lokalizacja'} className={'mb-5'}>
            <img
                ...
                alt={'close icon'}
                className={
                    'bg-red-400 rounded-2xl  flex relative top-2 right-1 cursor-pointer ' +
                    (!selectedAddress && !address ? 'hidden' : '')
                }
                width={15}
                height={15}
            ...
            <ul className={'search-results ' + (results.length > 0 ? 'block' : 'hidden')}>
