...
export const FiltersModal = ({
    ...
    return (
        <div
            className={
                'bg-color md:col-start-1 md:col-end-4 xl:col-start-2 xl:col-end-5 xl:mr-16 md:ml-6 ' +
                (hide ? 'hidden ' : null) +
                ' ' +
                (className ?? null)
            }
        >
            <div
                className={'flex flex-row justify-between bg-white order-2 md:hidden '}
            ...
            <FilterSection subTitle={'Czego szukasz'}>
                <WhatYouLookinFor developers={developers} principles={principles} onFilterClick={onFilterClick} />
            </FilterSection>
            <FilterSection subTitle={'Sortowanie'}>
                <Sorting options={sortOptions} selectedOption={order} onFilterClick={onFilterClick} />
            </FilterSection>
            ...
            {filtersSettings.principles ? (
                <WorkTypes workTypesList={workTypesList} onFilterClick={onFilterClick} workTypes={workTypes} />
            ) : null}

            <div className={'mb-4 md:hidden cursor-pointer'}>
                <div className={'application-button mt-5  p-8'} onClick={() => onClick(true)}>
                    {' '}
                    Filtruj
                </div>
            </div>
