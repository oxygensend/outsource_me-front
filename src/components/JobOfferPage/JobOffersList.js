...
import { ROLE_DEVELOPER, ROLE_PRINCIPLE } from '../../helpers/Roles';
...
export const JobOffersList = ({ filtersModal, jobOffers, getJobOffers, hasMore, developers, principles, order }) => {
    if (jobOffers) {
        return (
            <div
                className={
                    'md:col-start-5 md:col-end-13 xl:col-start-5 xl:col-end-11 md:mr-12 xl:mr-0  md:flex ' +
                    (!filtersModal ? 'hidden' : null)
                }
            >
                {order === 'for-you' &&
                (!tokenService.getLocalAccessToken() ||
                    (developers === true && tokenService.getUser().roles.includes(ROLE_DEVELOPER)) ||
                    (principles === true && tokenService.getUser().roles.includes(ROLE_PRINCIPLE))) ? (
                    ...
                    >
                        {principles
                            ? jobOffers.map((jobOffer, i) => {
                                  return <JobOfferCard key={i} jobOffer={jobOffer} />;
                              })
                            : jobOffers.map((jobOffer, i) => {
                                  return <DeveloperCard key={i} developer={jobOffer} />;
                              })}
                    </InfiniteScroll>
                )}
            </div>
