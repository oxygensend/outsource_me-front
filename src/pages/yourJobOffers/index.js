            ...
            getData('/api/users/' + id + '/job_offers?order=newest').then((data) => {
                ...
                data['hydra:member'].forEach((jobOffer, i) => {
                    if (!jobOffer.archived) {
                        ...
                        archived.push(<JobOfferManagementCard key={i} jobOffer={jobOffer} />);
                    }
                });
    ...
    return (
        <div className={'applications-wrapper pt-4'}>
            <div className={'text-center text-3xl mr-50 '}>
                ...
                <ButtonLink
                    class={
                        'outsourceme_button outsource_takeOff mt-4 sm:mt-0 relative transform -translate-x-1/2 left-1/2 sm:left-3/4 sm:absolute sm:translate-x-0 sm:top-4 '
                    }
                    value={'Nowa oferta'}
                    route={'/nowe-zlecenie'}
                />
