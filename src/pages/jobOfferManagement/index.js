...
export const JobOfferManagement = () => {
    ...
    const closeOfferContent = (offerName) => {
        return parse(
            'Czy na pewno chcesz zamknąć oferte na stanowisko <b>' +
                offerName +
                '</b>? Po zamknieciu oferty nie bedzie możliwości jej ponownego otwarcia.',
        );
    };
    return (
        <>
            <JobOfferManagementPage jobOffer={jobOffer} setShowModals={setShowModals} />
            ...
            {showModals.editOfferModal === true ? (
                <EditJobOfferModal setShowModals={setShowModals} jobOffer={jobOffer} />
            ) : null}
        </>
    );
};
